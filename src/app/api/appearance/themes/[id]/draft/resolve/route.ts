import { appearanceIdSchema, resolveDraftInputSchema } from "@/features/appearance/schemas/appearance-schema";
import { resolveAppearanceDraft } from "@/features/appearance/server/appearance-mutation-service";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const input = resolveDraftInputSchema.parse(
      await readBoundedJson(request, APPEARANCE_TECHNICAL_LIMITS.themeRequestBytes),
    );
    return apiData(await resolveAppearanceDraft(user.id, id, input));
  } catch (error) {
