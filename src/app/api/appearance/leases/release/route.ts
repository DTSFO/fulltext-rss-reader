...
import { releaseAppearanceLeases } from "@/features/appearance/server/appearance-db";
...
export async function POST(request: Request) {
  try {
    ...
    await releaseAppearanceLeases(user.id, input.holderToken, input.handles);
    return apiData({ released: true });
  } catch (error) {
